<x-base-layout>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @foreach ($permission->roles as $permission_role)
                                        <span> 
                                            <form action="{{ route('permissions.delete_roles', [$permission->id, $permission_role->id]) }}" method="post" enctype="multipart/form-data">
                                                {{ method_field('delete') }}
                                                {{ csrf_field() }}
                                                <button type="submit" class="badge badge-light-danger fs-7 m-1">{{$permission_role->name}}</button>
                                            </form>
                                        </span>
                                    @endforeach
                                    @endif
                                </td>
                                <td>{{ \Carbon\Carbon::parse($permission->created_at)->format('d M Y , g:i a')}}</td>
                                <td class="text-end">
                                    <!--begin::Assign Role-->
                                    <a class="btn btn-icon btn-active-light-primary w-30px h-30px" data-bs-toggle="modal" data-bs-target="#assign_{{$permission->id}}">
                                    ...
                                    <!--end::Assign Role-->
                                    <!--begin::Update-->
                                    <button class="btn btn-icon btn-active-light-primary w-30px h-30px me-3" data-bs-toggle="modal" data-bs-target="#update_{{$permission->id}}">
                                    ...
                                    <!--end::Update-->
                                    <!--begin::Delete-->
                                    <a class="btn btn-icon btn-active-light-danger w-30px h-30px" data-bs-toggle="modal" data-bs-target="#delete_{{$permission->id}}">
    ...
    @include('admin.permission.modals.create')
    @include('admin.permission.modals.delete')
    @include('admin.permission.modals.edit')
    @include('admin.permission.modals.assign')
</x-base-layout>